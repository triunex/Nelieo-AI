# Test the 3 killer demos for YC
# Using Ollama (slow but free)

$baseUrl = "http://localhost:8081"

Write-Host "🎯 Testing YC Demo Tasks with Ollama..." -ForegroundColor Cyan
Write-Host "⚠️  Note: Each demo takes ~40-60 seconds (LLaMA is slow but FREE)`n" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Result: $($response1.status)" -ForegroundColor $(if ($response1.status -eq 'completed') { 'Green' } else { 'Yellow' })
    Write-Host "Steps: $($response1.total_steps)" -ForegroundColor Gray
...
try {
    ...
    Write-Host "Result: $($response2.status)" -ForegroundColor $(if ($response2.status -eq 'completed') { 'Green' } else { 'Yellow' })
    Write-Host "Steps: $($response2.total_steps)" -ForegroundColor Gray
...
try {
    ...
    Write-Host "Result: $($response3.status)" -ForegroundColor $(if ($response3.status -eq 'completed') { 'Green' } else { 'Yellow' })
    Write-Host "Steps: $($response3.total_steps)" -ForegroundColor Gray
