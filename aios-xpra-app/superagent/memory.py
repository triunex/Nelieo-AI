...
class ShortTermMemory:
    ...
    def detect_loop(self, threshold: int = 3) -> bool:
        """
        Detect if agent is stuck in a loop - same action with same target
        ...
        recent_entries = list(self.memory)[-threshold:]
        recent_actions = [e.action.get('type') for e in recent_entries]
        
        # Check if all recent actions are the same type
        if len(set(recent_actions)) == 1:
            action_type = recent_actions[0]
            
            # For click actions, check if clicking same coordinates
            if action_type == 'click':
                coords = [(e.action.get('x'), e.action.get('y')) for e in recent_entries]
                # If clicking same coords multiple times, definitely a loop
                if len(set(coords)) <= 2:  # Allow 1-2 different attempts
                    logger.warning(f"Loop detected: clicking same coordinates {coords} {threshold} times")
                    return True
                else:
                    # Clicking different places - still exploring
                    logger.info(f"Trying different click locations {coords} - not a loop yet")
                    return False
            
            # For other actions (type, scroll, etc), check text/amount similarity
            if action_type in ['type', 'scroll']:
                targets = [e.action.get('text') or e.action.get('amount') for e in recent_entries]
                if len(set(str(t) for t in targets)) == 1:
                    logger.warning(f"Loop detected: {action_type} with same target {targets[0]} {threshold} times")
                    return True
            
            # Generic: same action type repeated without variation
            logger.warning(f"Loop detected: {action_type} repeated {threshold} times")
            return True
