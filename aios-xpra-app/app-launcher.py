    ...
    def launch_app(self, app_name, wait_time=2):
        ...
        try:
            ...
            if cmd == "google-chrome":
                full_cmd = f"google-chrome --no-sandbox --disable-dev-shm-usage --app={url} --new-window"
                self.window_controller.open_application(full_cmd, wait_time=wait_time)
            elif cmd == "microsoft-edge":
                full_cmd = f"microsoft-edge --no-sandbox --disable-dev-shm-usage --app={url} --new-window"
                self.window_controller.open_application(full_cmd, wait_time=wait_time)
            elif cmd == "slack":
